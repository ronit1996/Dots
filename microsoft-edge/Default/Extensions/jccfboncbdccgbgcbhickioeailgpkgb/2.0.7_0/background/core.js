...
const ddd = [
	...
	'||ad.outsidehub.com^',
	'||ad.outsidehub.*',
	'||ad.outsideb.ru/asd/asdd?var=1&b=1',
...
function isEasylistBlock(details) {
				...
				if ('domain' in extras) {
					...
					if (domain){
						if((initiator in domain) && (domain[initiator] == true)){
							...
							return false;// TODO: SKip domain with false more clever
						}
					}
					if (type in extras) {
						return true
...
function onBeforeRequest(details) {
	try {
		...
		if (blockingResponse.block) {
			...
			if (details.type == "main_frame" && !(excludes_block.includes(details.initiator))) {
			...
			if (pageData && pageData.isValidSite && !pageData.hasStands) {
				executeFileOnTab(details.tabId, "common/common.js", false, function() {
					executeFileOnTab(details.tabId, "common/common-content.js", false, function() {
						executeFileOnTab(details.tabId, "content/edge.js", false, function() {
			...
			checkAppExists(function(exists) {
				runOnActiveTab(function(tab) {
					if (exists) {
						...
						openTabWithUrl("https://new-prod.standsapp.org/pages/store-post-install/");
					}
				});
			});
		});
